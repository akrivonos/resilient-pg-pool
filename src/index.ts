 ...
 */
export function isTransientSocketError(e: unknown): boolean {
  const err = e as { code?: string; message?: string } | undefined;
  const code = err?.code;
  if (code === 'ECONNRESET' || code === 'EPIPE' || code === 'ETIMEDOUT') return true;
  return (
    !!err?.message &&
    /ECONNRESET|EPIPE|ETIMEDOUT|socket hang up|Connection terminated|timeout exceeded when trying to connect/i.test(
      err.message,
    )
  );
}
 ...
 * you override them.
 */
export function createResilientPool(options: ResilientPoolOptions = {}): ResilientPool {
  ...
  pool.on('error', (e) => {
    if (onIdleError) onIdleError(e as Error);
    else console.error('[resilient-pg-pool] idle client error (connection dropped):', (e as Error).message);
  });
  ...
  async function query<T extends Record<string, any> = any>(sql: string, params?: any[]): Promise<T[]> {
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      ...
      try {
        ...
        if (attempt < attempts - 1 && isTransientSocketError(e)) continue;
        throw e;
      }
      let errored = false;
      try {
        ...
        if (!errored) client.release();
      }
    }
    throw new Error('[resilient-pg-pool] query retry logic fell through unexpectedly');
  }

  async function queryOne<T extends Record<string, any> = any>(sql: string, params?: any[]): Promise<T | null> {
  ...
  async function transaction<T>(fn: (client: pg.PoolClient) => Promise<T>): Promise<T> {
    ...
    const attempts = retryDelaysMs.length;
    const acquireAndBegin = async (): Promise<pg.PoolClient> => {
      for (let attempt = 0; attempt < attempts; attempt++) {
        await sleep(retryDelaysMs[attempt]);
        let client: pg.PoolClient;
        try {
